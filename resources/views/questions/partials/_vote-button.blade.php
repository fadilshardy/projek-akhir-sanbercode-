<div class="col-sm-12 col-md-3 summary">
    <div class="row">
        ...
        <div class="col-sm-6 text-lg-left my-1">
            ...
            <button class="btn btn-md btn-vote {{!is_null($item->vote_status()) === 0 ? 'bg-danger' : ''}}" @guest
                onclick="alert('Login terlebih dahulu!')" @endguest>
            ...
            @else
            @auth
            <form action="/pertanyaan/{{$item->id}}/{{$item->vote_status() ? 'unvote/downvote ' : 'downvote'}}"
                method="POST">
                @csrf
                @endauth
                <button class="btn btn-md btn-vote {{!is_null($item->vote_status()) ? 'bg-danger' : ''}}">
                    <h5>{{$item->downvote_count()}}</h5>
                    <i class="fa fa-caret-down" aria-hidden="true"></i>
                </button>
                @auth
